...
function renderGauge(parent, firefighterId, type, initialNumber, unit) {
    ...
    var bg = g.append("path").datum({endAngle: tau}).style("fill", "#ddd").attr("d", arc);
    var fg = g.append("path").datum({endAngle: 0 * tau}).style("fill", color).attr("d", arc);
    var label = g.append("text").attr("dy", "0").attr("x", "0").attr("y", "0");
    var num = label.append("tspan").attr("class", "label-num").text(initialNumber).attr("x", "0");
    var unit = label.append("tspan").attr("class", "label-unit").text(unit).attr("x", "0").attr("dy", "1.2em");
