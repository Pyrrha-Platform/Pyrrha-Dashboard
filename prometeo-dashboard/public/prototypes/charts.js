...
function renderChart(parent, firefighterId, type, initialNumber, unit) {
    ...
    var svg = d3.select(parent).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom)
    ...
    d3.csv(csv,
        ...
        function(d){
            return { date: d3.utcParse("%Y-%m-%dT%H:%M:%S.%LZ")(d.date), value: d.value }
        },
